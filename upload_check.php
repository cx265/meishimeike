<?php
    ...
    $_upload_user = $_SESSION['user'];
    $_cxcontent = $_POST['content'];
    $_cxsort = $_POST['sort'];
...
if ($_POST['upload']){
    ...
    if ($_file['error']>0 || $_img['error']>0) {
        ...
        echo "<input type='button' value='返回' onclick=window.close()>";
        exit();
    }
    ...
    {
        echo "上传成功！\n";
        $_result = mysql_query("INSERT INTO video (video_type,video_content,video_uploader,theme,video_name,video_img,video_time,video_playcount,status) VALUES ('$_cxsort','$_cxcontent','$_upload_user','$_theme','$_save','$_saves',NOW(),'0','0')");
        if ($_result) {        
        echo "<script language='javascript'>alert('视频发布成功!');window.close();</script>";
        }
    }
    else
    {
    ...
    echo "<script language='javascript'>alert('视频没有被提交！');window.close();</script>";
}
?>